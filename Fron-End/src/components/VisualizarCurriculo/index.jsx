...
const VisualizarCurriculo = () => {
  ...
  const fetchCurriculoData = async () => {
    ...
    try {
      ...
      if (urlCandidatoId) {
        url = `http://localhost:8080/api/curriculos/by-candidate/${urlCandidatoId}`;
      } else {
        url = "http://localhost:8080/api/curriculos/by-candidate/me";
      }
  ...
  const confirmDelete = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `http://localhost:8080/api/curriculos/${curriculo.id}`,
        {
          method: "DELETE",
